...
public class HBAO : MonoBehaviour
{
    ...
    private void UpdateMaterialProperties()
    {
        ...
        _renderTarget.downsamplingFactor = GetDownsamplingFactor();
...
#if UNITY_EDITOR
        if (_hbaoCamera.orthographic)
            _hbaoShaderKeywords[1] = "ORTHOGRAPHIC_PROJECTION_ON";
        else
            _hbaoShaderKeywords[1] = _hbaoCamera.renderingPath == RenderingPath.DeferredShading || (_hbaoCamera.renderingPath == RenderingPath.UsePlayerSettings &&
                UnityEditor.PlayerSettings.renderingPath == RenderingPath.DeferredShading) ? "DEFERRED_SHADING_ON" : "__";
#else
        if (_hbaoCamera.orthographic)
            _hbaoShaderKeywords[1] = "ORTHOGRAPHIC_PROJECTION_ON";
        else
            _hbaoShaderKeywords[1] = _hbaoCamera.renderingPath == RenderingPath.DeferredShading ? "DEFERRED_SHADING_ON" : "__";
#endif
        if (_hbaoShaderKeywords[1] != "DEFERRED_SHADING_ON" && aoSettings.perPixelNormals == PerPixelNormals.GBuffer)
    ...
    private int GetDownsamplingFactor()
    {
        switch (generalSettings.resolution)
        {
